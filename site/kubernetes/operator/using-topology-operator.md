...
Messaging Topology Operator supports creating users with generated credentials and with provided credentials.
When creating a user with provided username and password, create a kubernetes secret object contains keys `username` and `password` in its Data field.
