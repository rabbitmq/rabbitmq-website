...
for beginners. And <a href="https://github.com/rabbitmq/rabbitmq-prometheus/blob/master/docker/prometheus.yml" target="_blank">here</a> example of docker prometheus config file on github (see `- job_name: 'rabbitmq-server'`).
...
One note: if after all steps you see random data instead real - go to datasources, mark 'prometheus' as default, remove dashboard and import again.
