 ...
 * `rabbitmq-diagnostics node_health_check` (née `rabbitmqctl node_health_check`) is **deprecated** and will be
   ...
   It has proven to be too prone to false positives, can consume a prohibitive amount of CPU resources and runtime scheduler,
   and be too opinionated for many technical operations teams.
 ...
 * [Definition import](https://www.rabbitmq.com/definitions.html) `management.load_definitions = /path/to/definitions/file.json` was not performed
   although its newer counterpart, `load_definitions = /path/to/definitions/file.json`, was.
 ...
 * `observer_cli` was upgraded [from 1.5.3 to 1.5.4]()
