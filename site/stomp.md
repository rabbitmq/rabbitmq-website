...
To permanently delete a durable topic, send an `UNSUBSCRIBE` frame for
...
session. Other sessions that use `reply-to:/temp-queue/foo` get different
queues.

When this message is received, the receiving client can obtain the
reply destination from the `reply-to` header of the `MESSAGE` frame.
Reply destinations thus received are generated by the broker (to ensure session-uniqueness)
and cannot be inferred from the temp queue name.
