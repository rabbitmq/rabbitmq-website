...
Quorum queues implement strict priority with no such interleaving: a sustained
stream of higher-priority messages will delay lower-priority ones indefinitely.
Workloads that need a guaranteed share of attention for lower-priority traffic
should use separate queues per priority class (see [Before Adopting Priority
Queues](#alternatives)).
