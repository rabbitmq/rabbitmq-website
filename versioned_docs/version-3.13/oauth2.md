...
This is a step-by-step guide on how to configure OAuth 2.0 authentication for the majority of
use-cases, such as:
...
- You want RabbitMQ to trigger OAuth 2.0 authentication in the management UI. This is known as
  *Service Provider initiated logon*. This is different from users needing to come to RabbitMQ
  already with a token obtained by other means.
...
Through this endpoint, RabbitMQ discovers all the necessary endpoints for a successful OAuth 2.0
authentication. The following identity providers are compliant with OpenId Connect:
...
:::info
It must be a HTTPS url. If your identity provider uses a certificate signed by a self-signed CA
...
RabbitMQ needs to know the username for logging purposes and also to display it in the management
UI. By default, RabbitMQ looks up the username in the token's attribute `sub`, and if not found it
uses `client_id`.
...
Given the following configuration, RabbitMQ would look for scopes in the attributes `scope` and
...
#### Step 6: OAuth 2.0 authentication completed for messaging protocols {#step7}
...
#### Step 7: Enable OAuth 2.0 on the management UI {#step8}
...
#### Step 8: Configure OAuth client_id {#step9}
...
#### Step 9: Configure which scopes the management UI requests from your identity provider {#step10}
...
You will always include the first two scopes `openid` and `profile` followed by any RabbitMQ scope.
...
:::info
Some identity providers may include more scopes than the requested scopes. It will all depend on the
actual identity provider implementation.
:::

## How it works {#how-it-works}

The OAuth 2 plugin must be activated (or [pre-configured](./plugins#enabled-plugins-file)) before it
can be used, like all other plugins:

```bash
rabbitmq-plugins enable rabbitmq_auth_backend_oauth2
```

Then it must be specified as one of the [authN and/or authZ backends](./access-control#backends).
The following example enables OAuth 2.0 authentication and authorization backends:
...
Usually, a numeric value is used as `index`, such as
...
the resource server's id. However, you can override it via the `id` attribute like in
...
Here is an example which configures two resources (`prod` and `dev`) where each resource is managed
...
resource_server of the JWT. By default, **RabbitMQ uses this field to validate the token** although
you can deactivate it by the `verify_aud` variable set to `false`. When it set to `true`, this
attribute must either match the `resource_server_id` variable or in case of a list, it must contain
the `resource_server_id`.
...
### Preferred user name claims {#preferred-username-claims}
...
*user-friendly user name*, you can configure one or several claims to extract the user name from the
token.
...
does return an `end_session_endpoint`, the management UI uses it over the configured endpoint.
:::
...
See this [table](#multiple-resource-servers-configuration) with all variables configurable for each
OAuth Provider.
...
See this [table](#multiple-oauth-providers-configuration) with all variables configurable for
each OAuth Provider.
