...
For full working configuration files, see the [rabbitmq-oauth2-tutorial](https://github.com/rabbitmq/rabbitmq-oauth2-tutorial/tree/next/conf) repository.
...
| `management.oauth_authorization_endpoint_params.audience` | `rabbitmq` | **Required since RabbitMQ 4.1.** Auth0 needs the `audience` parameter to include the correct `aud` claim in the token. |
| `management.oauth_token_endpoint_params.audience` | `rabbitmq` | **Required since RabbitMQ 4.1.** Same as above, for the token endpoint. |
