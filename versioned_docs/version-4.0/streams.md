 ...
 * Timestamp - a timestamp value specifying the approximate point in time to attach to the stream at.
 RabbitMQ uses the arrival time of each message to determine the attach offset, not any timestamp provided in the message itself.
 It will clamp to the closest preceding offset. If the timestamp is out of range for the stream, it will clamp to either the start or end of the stream respectively.
 Consumers can receive messages published before the specified timestamp. Applications should filter messages on the client side to process only those of interest.
