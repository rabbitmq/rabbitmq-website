 ...
 * The type of a queue
 * The [maximum number of priorities](./priority) of classic queues (quorum queues always provide a fixed 32-level range, also not configurable by a policy)
...
Every policy has a numeric priority associated with it. This priority can be specified when declaring
