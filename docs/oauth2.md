...
| `auth_oauth2.https.verify`                 | Configures [peer verification](ssl#peer-verification). Available values: `verify_none`, `verify_peer`. The default value is `verify_peer` if there are trusted CA installed in the OS or `auth_oauth2.https.cacertfile` is set.  <p/> **Note**: It replaces `auth_oauth2.https.peer_verification`. When both variables are present, this variable takes precedence over `auth_oauth2.https.peer_verification`
