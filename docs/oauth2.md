...
Since 4.1, you can configure a space-separated list of tokens' attributes. 

Take for instance this token below:
```json
{ 
  ...
  "authorization": {
...
and from `resource_access.account.roles` and `extra_scope` like this:
...
:::warning
Take into account that the scopes in `resource_access.account.roles` are 
not RabbitMQ recognized scopes. If you want to use those roles as RabbitMQ scopes,
you have to map them. See [steps 5)](#step5) and [6)](#step6).
:::
...
They all start with a prefix which, by default, must match the name of the resource you configured in step 1). 
