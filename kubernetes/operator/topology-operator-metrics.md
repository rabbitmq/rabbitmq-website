...
The metrics are available in Topology Operator v1.19+
...
* **Authentication and authorization are enabled** using Kubernetes TokenReview and SubjectAccessReview APIs
...
It is also possible to use an existing Service Account. Simply change the value of `--serviceaccount` flag to match your
...
2. Create a test pod with curl:

```bash
kubectl run curl-metrics --rm -it --restart=Never \
  --image=curlimages/curl:latest -n rabbitmq-system -- /bin/sh
```
