...
In the [first tutorial](./tutorial-one-elixir) we
...
```elixir
AMQP.Queue.declare(channel, "hello", durable: true, arguments: [{"x-queue-type", "quorum"}])
```
...
```elixir
AMQP.Queue.declare(channel, "task_queue", durable: true, arguments: [{"x-queue-type", "quorum"}])
```
...
Now we can move on to [tutorial 3](./tutorial-three-elixir) and learn how
