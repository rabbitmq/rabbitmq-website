...
confirms, and is registered as the first argument of
...
> This data structure is convenient for several reasons. It makes it easy to
> correlate a sequence number with a message (whatever the message data is)
> and to remove entries up to a given sequence id (to handle multiple
> confirms/nacks). Lastly, it supports concurrent access, because confirm
> callbacks are called in a thread owned by the client library, which should
> be kept separate from the publishing thread.
>
...
> they are usually more involved and do not belong in a tutorial.
...
performs poorly, as expected, but the results for asynchronous handling
